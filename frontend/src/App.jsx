...
import { 
  ...
  TrendingDown, Activity, Bell, ChevronRight, ChevronLeft, ChevronDown, 
...
const InterpretationBox = ({ icon: Icon, text, severity }) => (
  <div className={`flex items-start gap-2 px-4 py-3 border-l-2 text-[10px] font-sans leading-relaxed ${
...
export default function App() {
  ...
  const prophetSurv = current?.Prophet_Survival;
  ...
  const unreadAlertsDisplay = unreadCount;
      ...
      <div className="flex-1 overflow-y-auto tactical-grid no-scrollbar">
        <div className="max-w-[1400px] mx-auto px-6 py-6 space-y-6">
          ...
          <section className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
            ...
            <KpiCard label="Prophet Surv." value={prophetSurv !== undefined ? (prophetSurv >= 365 ? '365d+' : `${prophetSurv}d`) : '—'} icon={QueryStats} color={prophetSurv < 100 ? 'text-tactical-orange' : 'text-white'} sub="Trend Analysis" />
          </section>
              ...
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <ComposedChart data={data}>
                    ...
                    <Line type="stepAfter" dataKey="Prophet_Survival" name="Prophet Trend" stroke="#14b8a6" strokeWidth={2} dot={false} isAnimationActive={false} connectNulls />
                  </ComposedChart>
                </ResponsiveContainer>
              </div>
              <MethodologyBox label="Forecasting Methodology" 
                staticText="LSTM: 3-layer neural net trained on real macro data (Fed Funds, VIX). Takes 30-day sliding window of 7 liquidity features. Prophet: Additive model with crisis regressor and changepoint detection."
          ...
          <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-tactical-surface border border-tactical-border flex flex-col p-5 gap-4">
              <div className="flex justify-between items-center">
                <span className="text-[10px] font-bold text-tactical-dim uppercase tracking-widest">Market Intelligence</span>
                {current?.News_Type && (
                  <span className={`text-[9px] font-black px-2 py-0.5 ${
                    current.News_Type === 'negative' ? 'bg-tactical-red-bright text-white' :
                    current.News_Type === 'positive' ? 'bg-tactical-green text-black' :
                    'bg-tactical-dim text-white'
                  }`}>{current.News_Type.toUpperCase()}</span>
                )}
              </div>
              {current?.News_Headline ? (
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-3 text-[9px] font-mono text-tactical-dim">
                    ...
                    <span className="flex items-center gap-1"><AlertTriangle size={10} /> WEIGHT: {current.News_Weight?.toFixed(1)}</span>
                  </div>
                </div>
              ) : (
              ...
              <div className="mt-auto border-t border-tactical-border pt-4 flex flex-col gap-2">
                <div className="flex justify-between items-center text-[9px]">
                  <span className="text-tactical-dim">MARKET SENTIMENT</span>
