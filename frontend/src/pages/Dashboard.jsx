...
import {
  ...
  CreditCard as CreditScore, Sun, Moon, LogOut
} from 'lucide-react';
...
export default function Dashboard() {
  ...
  const { user, logout, setSimData } = useAuth();
  ...
  const stepEngine = async () => {
    try {
      ...
      setData(prev => {
        ...
        const trimmed = d.length > 90 ? d.slice(-90) : d;
        setSimData(trimmed);
        return trimmed;
      });
  ...
  return (
    <div className={`flex flex-col w-full h-screen bg-tactical-bg text-tactical-text font-mono overflow-hidden ${isLightMode ? 'light-mode' : ''}`}>
      ...
      {/* ═══ TOP BAR ═══ */}
      <header className="h-14 shrink-0 border-b border-tactical-border flex items-center justify-between px-6 bg-tactical-surface z-30">
        ...
        <div className="flex items-center gap-3">
                    ...
                    {systemAlerts.map(a => (
                      <div key={a.uid} className={`p-4 border-b border-tactical-border/30 hover:bg-white/5 transition-colors ${a.severity === 'critical' ? 'bg-tactical-red-bright/5' : ''}`}>
                        ...
                        {user?.role === 'executive' ? (
                          ...
                          a.solutions?.length > 0 && <span className="text-[8px] text-tactical-dim uppercase block mt-2 px-2 py-1 bg-black/40 border border-[#333] inline-block">Action Required by Executive</span>
                        )}
                      </div>
                    ))}
          ...
          <div className="h-4 w-px bg-tactical-border/50 mx-2"></div>
          <div className="flex items-center gap-2 text-[10px] font-bold text-tactical-dim uppercase bg-black/50 px-3 py-1.5 border border-tactical-border">
             <div className="w-1.5 h-1.5 bg-tactical-green rounded-full"></div> Role: {user && user.role ? user.role : 'Executive'}
          </div>
          <button onClick={logout || (() => navigate('/'))} className="text-tactical-dim hover:text-tactical-red-bright transition-colors ml-2" title="Logout">
            <LogOut size={16} />
          </button>
        </div>
      </header>
        ...
        {data.length === 0 ? (
          <div className="welcome-container">
            {user && user.type === 'enterprise' ? (
              <div className="max-w-4xl mx-auto mt-20 text-center px-6 pt-12">
                <Shield size={64} className="mx-auto text-tactical-dim mb-8" />
                <h1 className="text-3xl font-sans font-black text-white uppercase tracking-widest mb-4">Enterprise Node Authenticated</h1>
                <p className="text-[#888] leading-relaxed mb-8 max-w-lg mx-auto">Systems are active. Ready to ingest live streaming liquidity data and historic ledgers using your persistent API configuration.</p>
                <button onClick={startSim} className="bg-white text-black font-black uppercase text-xs px-8 py-4 tracking-[0.2em] shadow-[0_0_20px_rgba(255,255,255,0.1)] hover:bg-[#ccc] transition-all">
            ...
            <div className="mb-12">
              <div className="flex justify-between items-end mb-8">
                <div className="section-header-centered w-full text-left">
                  <div className="section-supertitle">GET STARTED</div>
                  <h2 className="section-title">New Instance Deployed.</h2>
                  <p className="section-subtitle">Since this is a fresh setup, we will use synthetic intraday models to demonstrate capability.</p>
                </div>
                <button onClick={startSim} className="flex-shrink-0 bg-tactical-green hover:bg-white text-black px-6 py-3 font-sans font-black tracking-widest text-[10px] uppercase transition-colors shadow-[0_0_15px_rgba(34,197,94,0.3)]">
                  Bootstrap Test Simulation
                </button>
              </div>
          ...
          <div className="max-w-[1600px] mx-auto px-6 py-6 space-y-8">
            ...
            {user?.role !== 'viewer' && activeSolutions.length > 0 && (
            ...
            {/* News Banner */}
            {user?.role !== 'viewer' && current?.News_Headline && (
            ...
            {user?.role !== 'viewer' ? (
              ...
              <section className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-6">
                <KpiCard label="Date" value={current?.Date || '—'} icon={Calendar} sub={`Day ${data.length}`} />
                <KpiCard label="Total Balance" value={current ? fmt((current.Daily_Deposits || 0) - (current.Daily_Withdrawals || 0)) : '—'} icon={AccountBalance} sub="Day Net Flow (Dep - With)" />
                <KpiCard label="L-to-D Ratio" value={ldr !== undefined ? `${ldr.toFixed(2)}%` : '—'} icon={CreditScore} sub="Funding Structure" />
              </section>
            )}
            ...
            {user?.role !== 'viewer' ? (
              ...
              <div className="mb-4"><h1 className="text-2xl font-sans font-black text-white uppercase tracking-tighter">Viewer Portal</h1><p className="text-sm text-tactical-dim">Cashflow Observer Interface</p></div>
            )}
